...
manifest. `--json` prints the same counts as data, in the shape its format
names. Anything worth an alert goes to stderr, so
...
def main() -> None:
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="print the counts as json instead of the block, in the shape"
        " its format names",
    )
