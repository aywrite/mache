...
# else is a branch, a tag or a commit. A script rather than action text so
# that shellcheck sees it and a test can run it directly.
set -euo pipefail
...
# secrets, and actions/resolve-ref/README.md says what that leaves open.
# Unset means this is not a runner at all.
